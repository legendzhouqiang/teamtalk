...
-(void)sendVoiceAgain:(DDMessageEntity *)message
{
    ...
    [[DDMessageSendManager instance] sendVoiceMessage:muData filePath:filePath forSessionID:message.sessionId isGroup:[message isGroupMessage] Message:message Session:[[SessionModule sharedInstance] getSessionById:message.sessionId] completion:^(DDMessageEntity *theMessage, NSError *error) {
        if (!error)
        {
