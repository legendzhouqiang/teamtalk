...
-(GroupAvatarImage *)getGroupImage:(GroupEntity *)group Block:(void(^)(UIImage *))block
{
    ...
    }else
    {
        [group.fixGroupUserIds enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            [[DDUserModule shareInstance] getUserForUserID:obj Block:^(DDUserEntity *user) {
                if (idx<=4) {
...
-(UIImage *)makeUserAvaterImage:(NSString *)userID
{
    ...
    [[DDUserModule shareInstance] getUserForUserID:userID Block:^(DDUserEntity *user) {
        NSData *data = [[PhotosCache sharedPhotoCache] photoFromDiskCacheForKey:[user getAvatarUrl]];
        UIImage *image = [UIImage imageWithData:data];
        newImage=image;
