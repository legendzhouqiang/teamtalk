...
+(DDMessageEntity *)makeMessage:(NSString *)content Module:(ChattingModule *)module MsgType:(DDMessageContentType )type
{
    ...
    MsgType msgType;
    if (module.sessionEntity.sessionType == SessionTypeSessionTypeSingle ) {
        msgType =MsgTypeMsgTypeSingleText;
    }else
    {
        msgType =MsgTypeMsgTypeGroupText;
    }
    DDMessageEntity* message = [[DDMessageEntity alloc] initWithMsgID:[DDMessageModule getMessageID] msgType:msgType msgTime:msgTime sessionID:module.sessionEntity.sessionID senderID:senderID msgContent:content toUserID:module.sessionEntity.sessionID];
